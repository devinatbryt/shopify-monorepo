...
export function makePersisted<
  S extends Signal<any> | [Store<any>, SetStoreFunction<any>],
>(signal: S, options?: PersistenceOptions<SignalType<S>, undefined>): S;
export function makePersisted<
  S extends Signal<any> | [Store<any>, SetStoreFunction<any>],
...
>(
  ...
  const storageOptions = (options as unknown as { storageOptions: O })
    .storageOptions;
  const serialize: (data: T) => string =
    options.serialize || JSON.stringify.bind(JSON);
  const deserialize: (data: string) => T =
    options.deserialize || JSON.parse.bind(JSON);
  // @ts-ignore
  ...
  if (init instanceof Promise)
    init.then((data) => unchanged && data && set(data as string));
    ...
    typeof signal[0] === "function"
      ? (value?: T | ((prev: T) => T)) => {
          ...
          const serialized: string | null | undefined =
            value != null
              ? (serialize(output) as string)
              : (value as null | undefined);
          options.sync?.[1](name, serialized);
          // @ts-ignore
          if (value != null)
            storage.setItem(name, serialized as string, storageOptions);
          // @ts-ignore
...
export const storageSync: PersistenceSyncAPI = [
  (subscriber: PersistenceSyncCallback) =>
    window.addEventListener("storage", (ev) =>
      subscriber(ev as PersistenceSyncData)
    ),
  () => {
...
export const messageSync = (
  channel: Window | BroadcastChannel = window
): PersistenceSyncAPI => [
  (subscriber: PersistenceSyncCallback) =>
  ...
  (subscriber: PersistenceSyncCallback) =>
    ws.addEventListener("message", (ev: MessageEvent) => {
      try {
        ...
        if (
          ["key", "newValue", "timeStamp"].every((item) =>
            Reflect.has(data, item)
          )
        ) {
          subscriber(data);
        }
 ...
 * ```
 */
export const multiplexSync = (
  ...syncAPIs: PersistenceSyncAPI[]
): PersistenceSyncAPI => [
